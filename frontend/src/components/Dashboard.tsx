...
export function Dashboard({ initialVerdicts, initialRegime, initialUpdated }: Props) {
  const { data: verdictsData } = useSWR<VerdictsResponse>("/api/verdicts", swrFetcher, {
    fallbackData: initialVerdicts,
  ...
  }, [verdicts, verdictFilter, riskFilter, search]);

  const sorted = useMemo(() => {
    return [...filtered].sort((a, b) => {
      const o = VERDICT_ORDER[a.verdict] - VERDICT_ORDER[b.verdict];
      if (o !== 0) return o;
      // Within same group, higher conviction first.
